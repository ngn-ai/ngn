...
def test_find_resume_branch_returns_true_when_branch_exists():
    """Should return True when ls-remote returns a non-empty ref line."""
    ref_output = "abc123\trefs/heads/ngn/NGN-17\n"
    with patch("subprocess.run", return_value=_make_result(stdout=ref_output)):
        result = find_resume_branch("https://github.com/example/repo.git", "ngn/NGN-17")
    assert result is True


def test_find_resume_branch_returns_false_when_branch_absent():
    """Should return False when ls-remote returns empty output (branch not found)."""
    with patch("subprocess.run", return_value=_make_result(stdout="")):
        result = find_resume_branch("https://github.com/example/repo.git", "ngn/NGN-99")
    assert result is False


def test_find_resume_branch_returns_false_on_error():
    """Should return False (not propagate) when subprocess raises an exception."""
